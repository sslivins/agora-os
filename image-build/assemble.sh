...
customize_rootfs() {
    ...
    # Version file (F17, Decision #2). Substituted at image-build time from
    # AGORA_OS_VERSION + AGORA_APP_FLOOR (asserted required at the top of
    # this script). os_updater/main.py:_read_current_version() parses the
    # agora_os_version key out of this file to drive the min_from_version
    # floor check; agora_app_floor is the minimum agora-app version this
    # rootfs supports (Decision #2, parallel agora-app OTA channel).
    # build-bundle.sh writes the same file with the same env vars so
    # OTA-applied slots have a version file too (otherwise the OTA target
    # would have NO /etc/agora/version at all).
