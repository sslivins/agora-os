...
# Write /etc/agora/version into the OTA bundle's rootfs. Mirrors the
# heredoc in assemble.sh:190. Without this, OTA-applied slot B has no
# version file and os_updater's _read_current_version raises on next
# update attempt. The pi-gen output does NOT include /etc/agora/version
# (the agora-os build owns it as image-build-time content), so we have
# to create the parent directory first.
echo "build-bundle: writing /etc/agora/version into root/..."
mkdir -p "${WORK}/bundle/root/etc/agora"
cat > "${WORK}/bundle/root/etc/agora/version" <<EOF
# Generated by build-bundle.sh at OTA-bundle-build time. Do not hand-edit.
agora_os_version=${AGORA_OS_VERSION}
agora_app_floor=${AGORA_APP_FLOOR}
EOF
chmod 0644 "${WORK}/bundle/root/etc/agora/version"
